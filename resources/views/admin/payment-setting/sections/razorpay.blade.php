    ...
    <div class="card">
        <div class="card-body border">
            <form action="{{ route('admin.razorpay-setting.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="">Razorpay URL</label>
                    <input name="razorpay_url" type="text" class="form-control"
                        value="{{ @$paymentGateway['razorpay_url'] }}">
                </div>
{{-- checkbox for testmode keys --}}
                        <div class="form-group">
                            <label for="">Razorpay Tess Mode</label>
                            <!-- Hidden input to send a default value when unchecked -->
    <input type="hidden" name="razorpay_test_mode" value="0">
    <input name="razorpay_test_mode" type="checkbox" class="custom-checkbox checkbox"
        value="1" {{ @$paymentGateway['razorpay_test_mode'] == 1 ? 'checked' : '' }}>
        <label for="razorpay_test_mode" class="custom-label">Are You Using Test Mode?</label>                  
                        </div>
                        <div class="form-group">
                            <label for="">Razorpay Test URL</label>
                            <input name="razorpay_test_url" type="text" class="form-control"
                                value="{{ @$paymentGateway['razorpay_test_url'] }}">
                        </div>
                        <div class="form-group">
                            <label for="">Razorpay Test Key</label>
                            <input name="razorpay_api_test_key" type="text" class="form-control"
                                value="{{ @$paymentGateway['razorpay_api_test_key'] }}">
                        </div>

                        <div class="form-group">
                            <label for="">Razorpay Test Secret Key</label>
                            <input name="razorpay_test_secret_key" type="text" class="form-control"
                                value="{{ @$paymentGateway['razorpay_test_secret_key'] }}">
                        </div>
{{-- testmode end --}}
                <div class="form-group">
